        ...
        try {
          ...
          showToast(`
            <div class="alert alert-info">
              ...
              Para acessar de outros computadores, use o link:<br>
              <a href="${url}" target="_blank" class="alert-link">${url}</a>
            </div>
          `, 10000);
        } catch (e) {
          showToast('<div class="alert alert-danger">Erro ao obter informações de rede.</div>');
        }
      });
    }
...
async function updateVendasPriceView(){
  ...
  if (lower.includes('mercado livre')) priceKey = 'Mercado Livre';
  const val = Number(prices[priceKey] ?? 0);
  vendaPrecoAtual = val;
